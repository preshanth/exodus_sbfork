...
def test_create_unpackaged_bundle_has_correct_proc_self_exe():
    ...
    try:
        binary_path = os.path.join(
            root_directory, "bin", os.path.basename(echo_proc_self_exe_glibc_32)
        )
...
@pytest.mark.parametrize(
    "fizz_buzz,expected_linker_path",
    [
